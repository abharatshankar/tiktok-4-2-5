        ...
        if Reachability.isConnectedToNetwork() == true
        {
            ...
            if  DEFAULT_HELPER.getId().isEmpty != true
            {
                if DEFAULT_HELPER.getId() != ""{
                    ...
                    self.UsersPostServiceCall(url: USER_SEARCH_API, userId: DEFAULT_HELPER.getId(), searchText: text)
                }
            ...
            else if  DEFAULT_HELPER.getfbId().isEmpty != true
            {
                if DEFAULT_HELPER.getfbId() != ""{
                    DispatchQueue.main.async {
                     ...
                     self.UsersPostServiceCall(url: USER_SEARCH_API, userId: DEFAULT_HELPER.getfbId(), searchText: text)
                }
...
//        self.navigationController?.pushViewController(details, animated: true)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "usersSearchTableViewCell", for: indexPath) as! usersSearchTableViewCell
        
         let myDic = self.searchArr[indexPath.row]  
            if let myname = myDic["fullName"] as? String{
                cell.userNameLbl.text = myname
            }
        
        if let mybio = myDic["bioData"] as? String{
            cell.bioDataLbl.text = mybio
        }
